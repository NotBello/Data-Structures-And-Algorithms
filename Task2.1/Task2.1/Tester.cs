...
namespace Vector
{
    ...
    public class Student : IComparable<Student>
    {
        ...
        {
            // Compare students based on Id in ascending order
    ...
    public class DescendingNameDescendingIdComparer : IComparer<Student>
    {
        public int Compare(Student p, Student q)
        {
            // Compare students based on name in descending order
            int nameComparison = string.Compare(q.Name, p.Name, StringComparison.Ordinal);

            // If names are equal, compare based on Id in descending order
            return nameComparison != 0 ? nameComparison : q.Id.CompareTo(p.Id);
        }
    }
